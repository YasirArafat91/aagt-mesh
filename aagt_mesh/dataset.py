...
#         # Handle variable-length (ragged) data
#         if field['type'] == 'dynamic_varlen':
#             # Decode lengths
#             length_bytes = features['length_'+key].values
#             length = tf.io.decode_raw(length_bytes, tf.int64)
#             length = tf.reshape(length, [-1])
            
#             # Reshape data to [total_elements, inner_dim]
#             inner_dim = field['shape'][1] if len(field['shape']) > 1 else 1
#             data = tf.reshape(data, [-1, inner_dim])
            
#             # Create ragged tensor along the first dimension
#             data = tf.RaggedTensor.from_row_lengths(data, row_lengths=length)
