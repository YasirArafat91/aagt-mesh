# `aagt_mesh/` — AAGT-Mesh model and training/eval pipeline

`aagt_mesh` is the model proposed in the paper: it keeps the MeshGraphNets
...
| `TransformerBlock.py` | One pre-norm Transformer encoder layer over node tokens (self-attention + FFN + residual + LayerNorm), with optional additive attention bias | **new (AAGT-Mesh)** |
| `MultiHeadSelfAttention.py` | Scaled dot-product multi-head self-attention with optional `[N, N]` bias | **new (AAGT-Mesh)** |
| `gpe.py` | `GeometricPositionalEncoding`: MLP projection of mesh node coordinates into model-dim positional features | **new (AAGT-Mesh)** |
...
Use `--model=cfd` with `cylinder_flow` / `airfoil` and `aagt_mesh.plot_cfd` for
the CFD domain. `run_cfd_m.sh` / `run_cloth_m.sh` run short smoke tests; the
