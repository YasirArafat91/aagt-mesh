...
| `TransformerBlock.py` | One pre-norm Transformer encoder layer over node tokens (self-attention + FFN + residual + LayerNorm), with optional additive attention bias; also defines `GeometricPositionalEncoding` (MLP projection of mesh node coordinates into model-dim positional features) | **new (AAGT-Mesh)** |
