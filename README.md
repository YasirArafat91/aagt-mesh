...
This repository (short name: **aagt-mesh**) accompanies the paper
*"Adjacency-Aware Graph Transformers for Learning Mesh Dynamics"*. It builds on
...
processor (**AAGT-Mesh**): edge features are still built from sender/receiver node
...
(`aagt_mesh/core_model.py`, `aagt_mesh/TransformerBlock.py`,
`aagt_mesh/MultiHeadSelfAttention.py`, `aagt_mesh/gpe.py`).

The model is evaluated on the same simulation domains as MeshGraphNets:

- `cylinder_flow` — CFD (`aagt_mesh/cfd_model.py`, `aagt_mesh/cfd_eval.py`, `aagt_mesh/plot_cfd.py`)
- `flag_simple` — cloth (`aagt_mesh/cloth_model.py`, `aagt_mesh/cloth_eval.py`, `aagt_mesh/plot_cloth.py`)
...
> baseline (`mgn`) alongside AAGT-Mesh for comparison. Only the AAGT-Mesh implementation
> (`aagt_mesh/`) is included in this repository — the `meshgraphnets` package must
...
```
.
├── aagt_mesh/                        # AAGT-Mesh model, training/eval pipeline (see aagt_mesh/README.md)
├── plot_three_panel.py               # Ground truth / MGN / AAGT-Mesh comparison figure (flag_simple)
├── plot_cylinder_three_panel.py      # Ground truth / MGN / AAGT-Mesh comparison figure (cylinder_flow)
├── job_s_mgn_vs_mgt_cfd.sh           # SLURM job: train+rollout MGN vs AAGT-Mesh on cylinder_flow
├── job_s_mgn_vs_mgt_cloth.sh         # SLURM job: train+rollout MGN vs AAGT-Mesh on flag_simple
...
```bash
mkdir -p ${DATA}
bash aagt_mesh/download_dataset.sh flag_simple ${DATA}
```
...
Train AAGT-Mesh on the cloth domain:

```bash
python -m aagt_mesh.run_model --mode=train --model=cloth \
...
```bash
python -m aagt_mesh.run_model --mode=eval --model=cloth \
    --checkpoint_dir=${DATA}/chk --dataset_dir=${DATA}/flag_simple \
...
```bash
python -m aagt_mesh.plot_cloth --rollout_path=${DATA}/rollout_flag.pkl
```

Use `--model=cfd` with a CFD dataset (`cylinder_flow` or `airfoil`) and
`aagt_mesh.plot_cfd` for the CFD domain.

### Quick smoke tests

`aagt_mesh/run_cfd_m.sh` and `aagt_mesh/run_cloth_m.sh` run a short train → rollout →
plot cycle to sanity-check the pipeline before launching a full job:

- `aagt_mesh/run_cfd_m.sh` — 10 training steps on `DATA/cylinder_flow`, 1 rollout,
  then `aagt_mesh.plot_cfd`.
- `aagt_mesh/run_cloth_m.sh` — 50,000 training steps on `Data/flag_simple`, 50
  rollouts, then `aagt_mesh.plot_cloth`. It also calls `test.plot_gt_vs_pred`,
...
3. Trains AAGT-Mesh the same way (`python -m aagt_mesh.run_model --mode=train ...`)
   for 200,000 steps and 100 rollouts, saving to
   `Data/<dataset>/aagt_mesh/checkpoint` and `Data/<dataset>/aagt_mesh/rollout.pkl`.
